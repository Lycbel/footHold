...
public class AuthDB extends BasicDB {
    ...
    public long signup(User usr){
        ...
        try {
            Connection conn = getConn();
            PreparedStatement pstmt = conn.prepareStatement("insert into  users (userID,firstName,lastName,address,email,password) VALUES (?,?,?,?,?,?);");
            ...
            pstmt.setString(5, usr.getUserEmail());
            pstmt.setString(6, MD5(usr.getPassWord()));
            pstmt.executeUpdate();
